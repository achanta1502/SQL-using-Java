	...
	public static void createTable(String table, String[] col){
		try{	
			...
			for(int p = 1; p <= numOfPages; p++){
				int rm = Indexing.getRightMost(file, p);
				if(rm == 0)
					page = p;
			}
			
			int[] keys = Indexing.getKeyArray(file, page);
			int l = keys[0];
			...
			keys = Indexing.getKeyArray(file, page);
			l = keys[0];
			for(int i = 0; i < keys.length; i++)
				if(keys[i]>l)
					l = keys[i];
...
public static String[] retrieveValues(RandomAccessFile file, long loc){
	...
	public static void drop(String table){
		try{
			...
			for(int page = 1; page <= numOfPages; page ++){
				...
				if(fileType == 0x0D)
				{
					short[] cellsAddr = Indexing.getCellArray(file, page);
					int k = 0;
					for(int i = 0; i < cellsAddr.length; i++)
					{
						long loc = Indexing.getCellLoc(file, page, i);
						String[] vals = retrieveValues(file, loc);
						String tb = vals[1];
						if(!tb.equals(table))
						{
							Indexing.setCellOffset(file, page, k, cellsAddr[i]);
							k++;
						}
					}
					Indexing.setCellNumber(file, page, (byte)k);
				}
				else
					continue;
			}
	...
	public static void delete(String table, String[] cmp){
		...
		for(int p = 1; p <= numPages; p++)
			if(Indexing.hasKey(file, p, key)&Indexing.getPageType(file, p)==0x0D){
				page = p;
				break;
			}
	...
	public static void update(String table, String[] cmp, String[] set){
		try{
			...
			for(int i = 0; i < keys.length; i++)
				if(keys[i] == key)
					x = i;
			int offset = Indexing.getCellOffset(file, page, x);
			long loc = Indexing.getCellLoc(file, page, x);
			...
			Indexing.updateLeafCell(file, page, offset, plsize, key, stc, values);
	...
	public static void insertInto(RandomAccessFile file, String table, String[] values){
		...
		if(page != 0)
			if(Indexing.hasKey(file, page, key)){
		...
		int offset = Indexing.checkLeafSpace(file, page, cellSize);


		if(offset != -1){
			Indexing.insertLeafCell(file, page, offset, plSize, key, stc, values);
		}else{
			Indexing.splitLeaf(file, page);
			insertInto(file, table, values);
		}
	}
...
public static int searchKeyPage(RandomAccessFile file, int key){
		...
		try{
			...
			for(int page = 1; page <= numPages; page++){
				...
				if(pageType == 0x0D){
					...
					if(keys.length == 0)
						return 0;
					int rm = Indexing.getRightMost(file, page);
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, String[] type, Updation buffer){
		try{
			...
			for(int page = 1; page <= numOfPages; page++){
				...
				byte pageType = file.readByte();
					...
					byte numOfCells = Indexing.getCellNumber(file, page);
	...
	public static void filter(RandomAccessFile file, String[] cmp, String[] columnName, Updation buffer){
		try{
